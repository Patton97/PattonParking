...
CircleGizmo::CircleGizmo(GameObject* parent, sf::Vector2f positionOffset, float rotationOffset)
{
    this->setParent(parent);
    this->m_positionOffset = positionOffset;
    this->m_rotationOffset = rotationOffset;
    point = sf::Vertex(this->m_parent->getPosition() + positionOffset, sf::Color::Green);
}
...
{
    this->point.position = this->m_transform->getPosition();
